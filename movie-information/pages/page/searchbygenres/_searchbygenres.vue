...
<div style="background:rgba(38, 38, 38, 0.65);">
    ...
    <div class="box">
        <div class="menu_list">
            <p class="topic" style="color: #FADB14;">Category : {{genres_text}}</p>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            search_params: +this.$route.params.searchbygenres,
            search: {},
            genres: {},
            genres_text: null
        }
    ...
    methods: {
        ...
        async getData(current=1) {
            ...
            await axios
                .get(`https://api.themoviedb.org/3/discover/movie?api_key=38c75e40ea604b41a0a54c8b05a806fc&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${this.current}&with_genres=${this.search_params}`)

                .then(response => {
                    this.search = response.data

                    console.log("xx", this.genres_text)
                })
                .catch(err => {
                    console.log(err)
                })
            await axios
                ...
                .then(response => {
                    ...
                    this.genres_text = this.genres.filter(el => el.id === this.search_params).pop().name
                    console.log('xx', this.genres_text)
                })
                .catch(err => {
                    console.log(err)
